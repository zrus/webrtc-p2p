...
impl Pipeline {
    pub fn init() -> Result<Self, anyhow::Error> {
        let pipeline = gst::parse_launch(
            "videotestsrc pattern=ball is-live=true ! videoconvert ! x264enc ! rtph264pay config-interval=-1 ! application/x-rtp,media=video,encoding-name=H264,payload=100,clock-rate=90000 ! udpsink host=127.0.0.1 port=5004",
        )
