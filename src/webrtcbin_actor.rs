...
impl WebRTCPipeline {
    fn create_client() -> Result<Self, anyhow::Error> {
        let pipeline = gst::parse_launch("
            webrtcbin name=webrtcbin turn-server=turn://tel4vn:TEL4VN.COM@turn.tel4vn.com:5349?transport=tcp bundle-policy=max-bundle
    ...
    fn on_incoming_decodebin_stream(&self, pad: &gst::Pad) -> Result<(), anyhow::Error> {
        ...
        let sink = if name.starts_with("video/") {
            ...
            gst::parse_bin_from_description("queue ! videoconvert ! glimagesink", true)?
        } else {
